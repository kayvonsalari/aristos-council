...
def check_narration(narrative: str, table: dict) -> list[str]:
    ...
    "factors": {factor_key: rank}, "score": float|None}``. Each sentence is checked three ways
    — a word-superlative claim (best/worst/second-*) against a citation or its subject, any
    digit ordinals bound to the factor each names, and (when ``score`` is supplied) a cited
    combined rank-SUM value against that authoritative score. Correct ordinal statements (in
    any factor order) pass untouched; ambiguous or hedged sentences are left alone — the check
    never invents a contradiction, and never rewrites the prose.
