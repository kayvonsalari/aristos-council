...
def test_a_suggested_universe_that_no_longer_exists_falls_back_to_its_id():
    # FUND-UI-2 deleted the shipped cohorts a few strategies still name. That is inert
    # by design — an unresolvable id is shown as itself and skipped in the selectors,
    # never a crash — and it comes back to life if a list is saved under that id.
    detail = strategy_detail("financials_v1", STRAT_DIR)
