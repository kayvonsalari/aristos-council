...
| `Summary withheld: number not in the facts: 120` · `Summary withheld: forbidden word: "attractive"` · `Summary withheld: 342 words` | The summary was written and then FAILED a check — a number that is not in the run, an advice word, over the 300-word limit, an empty field, or a company the run does not carry. (`reader_check.py`, READER-1) | **Not a broken feature and not a silent omission.** The note was attempted, checked and refused; the line exists so a reader knows that happened. Nothing is corrected or trimmed to make it publishable — the same abstention rule the rest of the report follows. |
